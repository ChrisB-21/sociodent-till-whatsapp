...
import { Calendar, Clock, Search, RefreshCw, UserCheck, AlertTriangle, Download, FileText } from 'lucide-react';
...
const AdminAppointmentsManager = () => {
  ...
  const getUnassignedCount = () => appointments.filter(a => a.status === 'pending' && !a.doctorId).length;  // Format date for display
  ...
  const handleFileDownload = async (reportFile: Appointment['reportFile']) => {
    if (!reportFile) return;

    try {
      // Create a link element and trigger download
      const link = document.createElement('a');
      link.href = reportFile.fileUrl;
      ...
      toast({
        ...
        description: 'Failed to download the report file. Please try again.',
        variant: 'destructive'
      });
    }
  };
                ...
                <tr key={appointment.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap">
                    {appointment.reportFile ? (
                      <div className="flex items-center space-x-2">
                        <div className="flex flex-col">
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="text-sm font-medium text-gray-900 truncate max-w-32" title={appointment.reportFile.fileName}>
                              {appointment.reportFile.fileName}
                            </span>
                          </div>
                          <span className="text-xs text-gray-500">
                            {formatFileSize(appointment.reportFile.fileSize)}
                          </span>
                        </div>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleFileDownload(appointment.reportFile)}
                      ...
                      <span className="text-gray-400 text-sm italic">No report</span>
                    )}
                  </td>
                  <td className="px-4 py-4 whitespace-nowrap">
